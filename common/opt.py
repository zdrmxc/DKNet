...
class opts():
    ...
    def init(self):
        ...
        self.parser.add_argument('--model', default='DKNet', type=str)
        ...
        self.parser.add_argument('--drop', default=0.1, type=float)
    ...
    def parse(self):
        ...
        if self.opt.dataset in ['mpii_3dhp', 'mpi_inf_3dhp', '3dhp']:
            self.opt.subjects_train = 'S1,S2,S3,S4,S5,S6,S7,S8'
            self.opt.subjects_test = 'TS1,TS2,TS3,TS4,TS5,TS6'
        else:
            self.opt.subjects_train = 'S1,S5,S6,S7,S8'
            self.opt.subjects_test = 'S9,S11'

        if self.opt.train:
            self.opt.checkpoint = os.path.join('ckpt', self.opt.model_name)
            os.makedirs(self.opt.checkpoint, exist_ok=True)
